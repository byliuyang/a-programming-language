...
(define (desugar-class [c : ClassS]) : ExprC
  (lamC (classS-constructor-vars c)
        (appC
         (lamC (list 'parent-class 'self)
               (appC
                (lamC (map (lambda (def)
                             (defS-name def))
                                (classS-private-vars c))
                      (appC (let ([pub-vars (map (lambda (def)
                                                   (defS-name def))
                              ...
                              (lamC pub-vars
                                    (appC
                                     (lamC (list 'dummy)
                                           (idC 'self))
                                     (list (setC 'self (lamC (list 'msg)
                                                             (varcaseC 'msg 
                                                                       (append (map (lambda (method)
                                                                                      (optC (defS-name method)
                                                                                            (desugar (defS-val method))))
                                                                                    (classS-methods c))
                                                                               (foldl (lambda (pub-var result)
                                                                                        (append (generate-getter-setter pub-var)
                                                                                                result))
                                                                                      empty
                                                                                      pub-vars))
                                                                       (appC (idC 'parent-class)
                                                                             (list (idC 'msg))))))))))
                              (map (lambda (def)
                                     (desugar (defS-val def)))
                                   (classS-public-vars c))))
                      (map (lambda (def)
                             (desugar (defS-val def)))
                           (classS-private-vars c))))
         (list (make-parent c) (numC -12323232321)))))

; Make parent object
(define (make-parent [c : ClassS]) : ExprC
  (case (classS-parent c)
    [(Object)(lamC (list 'msg)
                   (varcaseC 'msg empty (idC 'notfound)))]
    [else (appC (classidC (classS-parent c))
                (map (lambda (var)
                       (idC var))
                     (classS-constructor-vars c)))]))

; Getter and Setter definition
(define-type-alias GetterSetter (listof OptionC))
(define-type-alias Getter OptionC)
(define-type-alias Setter OptionC)

; Generate getters and setters helpers
(define (generate-getter-setter [field : symbol]) : GetterSetter
  (list (generate-getter field) (generate-setter field)))

(define (generate-getter [field : symbol]) : Getter
  (optC (generate-symbol 'get field) (lamC empty (idC field))))

(define (generate-setter [field : symbol]) : Setter
  (optC (generate-symbol 'set field) (lamC (list 'val) (setC field (idC 'val)))))

(define (generate-symbol [prefix : symbol] [field : symbol]) : symbol
  (string->symbol (string-append (string-append (symbol->string prefix) "-") (symbol->string field))))
...
(define (desugar [as : ExprS]) : ExprC
  (type-case ExprS as
    ...
    [varcaseS (var options elsecase) (varcaseC var (map (lambda (opt) 
                                               ...
                                               (desugar elsecase))]
    ; edit sendS
    [sendS (obj msg args) (appC (appC (desugar obj) (list (msgidC msg)))
                                (map (lambda (arg) (desugar arg)) args))]
    ; edit newS
    [newS (classname args) (appC (classidC classname) (map (lambda (arg) (desugar arg)) args))]
    ))
